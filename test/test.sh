...
if [[ "$OUT" == *"default test no_block"* ]]; then
    echo "[pass] no_block"
else
    echo "$OUT"
...
# GET Crons
OUT=$(curl -sSk -b "$COOKIE_FILE" "$URL/v1/pal/crons")
if [[ "$OUT" == *"no_auth"* ]]; then
    echo "[pass] crons/get"
else
    echo "$OUT"
